...
export const Button = ({
  ...
  const baseStyles =
    'inline-flex items-center justify-center font-semibold rounded-lg transition-all duration-200 focus:outline-none px-5 py-2.5 text-sm';
  const variants = {
    primary:
      'bg-[#2562EB] text-white hover:bg-[#1d4ed8] focus:ring-4 focus:ring-blue-200 disabled:bg-gray-300 disabled:text-gray-500',
    secondary:
      'bg-white text-[#2562EB] border border-[#2562EB] hover:bg-blue-50 focus:ring-4 focus:ring-blue-100',
    outline:
      'border border-gray-300 text-[#0F172A] hover:bg-gray-50 focus:ring-4 focus:ring-gray-100',
    danger:
      'bg-[#EF4444] text-white hover:bg-red-600 focus:ring-4 focus:ring-red-100',
  };
  return (
    <button
      type={type}
...
export const Input = ({
  ...
  <div className="w-full flex flex-col gap-1.5">
    ...
    <input
      ...
      className={`w-full px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#2562EB]/20 focus:border-[#2562EB] transition-all duration-200 ${error ? 'border-[#EF4444]' : 'border-gray-300'} ${className}`}
      {...props}
    />
    {error && (
...
export const Checkbox = ({ label, id, ...props }) => (
  <div className="flex items-center gap-2">
    <input
      type="checkbox"
      id={id}
      className="w-4 h-4 text-[#2562EB] border-gray-300 rounded focus:ring-[#2562EB]"
      {...props}
    />
    {label && (
...
export const Radio = ({ label, name, id, ...props }) => (
  <div className="flex items-center gap-2">
    <input
      ...
      id={id}
      name={name}
      className="w-4 h-4 text-[#2562EB] border-gray-300 focus:ring-[#2562EB]"
      {...props}
    />
    {label && (
      <label htmlFor={id} className="text-sm text-gray-700 select-none">
...
export const Toggle = ({ checked, onChange, label }) => (
  <label className="flex items-center gap-3 cursor-pointer select-none">
    <div className="relative">
      ...
      />
      <div
        className={`w-10 h-6 rounded-full transition-colors ${checked ? 'bg-[#2562EB]' : 'bg-gray-300'}`}
      ></div>
      <div
...
export const Badge = ({ children, status = 'success', className = '' }) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${statuses[status]} ${className}`}
    >
      {children}
    </span>
  );
...
export const ProductCard = ({ title, price, image, category, tag }) => (
  <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow relative">
    {tag && (
      <Badge status="warning" className="absolute top-3 left-3">
        {tag}
      </Badge>
    )}
    ...
    <h4 className="font-semibold text-gray-800 text-sm mt-1 line-clamp-1">
      {title}
    </h4>
...
export const CategoryCard = ({ name, count, icon }) => (
  <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4 hover:border-[#2562EB] cursor-pointer transition-colors">
    <div className="w-12 h-12 bg-blue-50 rounded-lg flex items-center justify-center text-xl text-[#2562EB]">
      {icon || '⚙️'}
    </div>
    <div>
...
export const DashboardCard = ({
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm flex items-center justify-between">
      ...
      <div
        className={`w-12 h-12 ${colors[status]} text-white rounded-xl flex items-center justify-center text-xl shadow-sm`}
      >
        {icon}
      </div>
    </div>
  ...
  <div className="flex border-b border-gray-200 w-full gap-6">
    {tabs.map((tab) => (
      <button
        ...
        className={`pb-3 text-sm font-semibold border-b-2 transition-colors ${activeTab === tab.id ? 'border-[#2562EB] text-[#2562EB]' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
      >
        {tab.label}
      </button>
    ))}
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden bg-white">
      <button
        ...
        className="w-full px-4 py-3 flex items-center justify-between font-semibold text-sm text-gray-700 bg-gray-50 hover:bg-gray-100 transition-colors"
      >
        <span>{title}</span>
        <span>{isOpen ? '−' : '+'}</span>
      </button>
      {isOpen && (
        <div className="p-4 border-t border-gray-200 text-sm text-gray-600 bg-white">
          {children}
        </div>
      )}
    </div>
