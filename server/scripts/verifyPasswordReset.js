...
const runTests = async () => {
  ...
  try {
    ...
    assert.ok(
      error1 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    assert.equal(error1.statusCode, 400, 'Should return 400 Bad Request');
    assert.ok(
      error1.message.includes('Password must be between'),
      'Should have correct length validation message'
    );
    ...
    // ----------------------------------------------------
    console.log(
      'Running Test 2: registerUser with password missing special character...'
    );
    const req2 = {
      body: {
        name: 'Test User',
    ...
    assert.ok(
      error2 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    ...
    assert.equal(
      ...
      'Should have correct complexity validation message'
    );
    ...
    assert.ok(
      error3 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    assert.equal(error3.statusCode, 400, 'Should return 400 Bad Request');
    assert.ok(
      error3.message.includes('Password must be between'),
      'Should validate password before processing OTP'
    );
    ...
    assert.ok(
      error4 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    ...
    assert.ok(
      error5 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    assert.equal(error5.statusCode, 400, 'Should return 400 Bad Request');
    assert.ok(
      error5.message.includes('Password must be between'),
      'Should validate new password structure'
    );
    ...
    assert.equal(
      updatedOtpUser.forgot_password_failedAttempts,
      0,
      'Should reset failed attempts'
    );
    assert.equal(
      updatedOtpUser.forgot_password_lockUntil,
      null,
      'Should reset lockUntil'
    );
    ...
    assert.ok(
      error7 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    ...
    assert.ok(
      error8 instanceof ErrorHandler,
      'Should return ErrorHandler error'
    );
    ...
    const { res: res9, err: error9 } = await callController(
      verifyEmailOtp,
      req9
    );
    ...
    assert.notEqual(
      updatedExpiredUser.login_otp,
      emailOtpHashed,
      'Should hash the newly generated OTP'
    );
    ...
    assert.notEqual(
      updatedResendUser.login_otp,
      emailOtpHashed,
      'Should set a new hashed OTP'
    );
    assert.ok(
      updatedResendUser.login_otp.startsWith('$2a$') ||
        updatedResendUser.login_otp.startsWith('$2b$'),
    ...
    // ----------------------------------------------------
    console.log(
      'Running Test 13: updateUserDetails response schema consistency...'
    );
    const req13 = {
    ...
    const { res: res13, err: error13 } = await callController(
      updateUserDetails,
      req13
    );
    ...
    assert.equal(
      res13.body.data,
      undefined,
      'Should NOT contain the duplicate data key'
    );
    ...
    // ----------------------------------------------------
    console.log(
      'Running Test 14: MANAGER role soft-delete visibility check...'
    );
    ...
    const { res: res14List, err: error14List } = await callController(
      getAllUsers,
      req14List
    );
    assert.equal(error14List, null);
    const hasSoftDeleted = res14List.body.data.some(
      (u) => u.email === 'softdeleted@example.com'
    );
    assert.equal(
      hasSoftDeleted,
      false,
      'MANAGER should not see soft-deleted users in listing'
    );
    ...
    assert.equal(
      error14Get.statusCode,
      404,
      'MANAGER should get 404 for soft-deleted user'
    );
    ...
    // ----------------------------------------------------
    console.log(
      'Running Test 15: MANAGER role cannot manage soft-deleted users...'
    );
    const req15Delete = {
      user: { role: 'MANAGER', _id: dummyUser._id },
      params: { id: softDeletedUser._id },
    };
    const { err: error15Delete } = await callController(
      deleteUser,
      req15Delete
    );
    assert.ok(error15Delete instanceof ErrorHandler);
    assert.equal(
      error15Delete.statusCode,
      404,
      'MANAGER should get 404 on deleting soft-deleted user'
    );
    ...
    const { err: error15Status } = await callController(
      updateUserStatus,
      req15Status
    );
    assert.ok(error15Status instanceof ErrorHandler);
    assert.equal(
      error15Status.statusCode,
      404,
      'MANAGER should get 404 on updating status of soft-deleted user'
    );
    ...
    // ----------------------------------------------------
    console.log(
      'Running Test 16: ADMIN role can see and manage soft-deleted users...'
    );
    const req16List = {
      user: { role: 'ADMIN', _id: dummyUser._id },
      query: { page: 1, limit: 10, search: '' },
    };
    const { res: res16List, err: error16List } = await callController(
      getAllUsers,
      req16List
    );
    assert.equal(error16List, null);
    const hasSoftDeletedAdmin = res16List.body.data.some(
      (u) => u.email === 'softdeleted@example.com'
    );
    assert.equal(
      hasSoftDeletedAdmin,
      true,
      'ADMIN should see soft-deleted users in listing'
    );
    ...
    const { res: res16Get, err: error16Get } = await callController(
      getSingleUser,
      req16Get
    );
    assert.equal(error16Get, null);
    assert.equal(
      res16Get.body.data.email,
      'softdeleted@example.com',
      'ADMIN should fetch soft-deleted user'
    );
    ...
    const { res: res16Status, err: error16Status } = await callController(
      updateUserStatus,
      req16Status
    );
    assert.equal(error16Status, null);
    assert.equal(
      res16Status.body.user.status,
      'Suspended',
      'ADMIN should successfully update status of soft-deleted user'
    );
    ...
    // ----------------------------------------------------
    console.log(
      'Running Test 17: verify auth middleware precedes upload on /upload-avatar route...'
    );
    ...
    assert.equal(
      middlewareNames[0],
      'auth',
      'auth middleware must execute BEFORE multer file processing'
    );

    console.log(
      'All password reset/validation/OTP/Email verification/User details/Admin visibility tests passed successfully.'
    );
  } finally {
